...
while j < 100:
    i = int(j * (1 / h))
    k0 = h * y[i]
    l0 = h * (-numpy.sign(y[i])*beta - numpy.sin(x[i]))
    k1 = h * (y[i] + 0.5 * l0)
    l1 = h * (-numpy.sign(y[i])*beta - numpy.sin(x[i] + 0.5 * k0))
    k2 = h * (y[i] + 0.5 * l1)
    l2 = h * (-numpy.sign(y[i])*beta - numpy.sin(x[i] + 0.5 * k1))
    k3 = h * (y[i] + l2)
    l3 = h * (-numpy.sign(y[i])*beta - numpy.sin(x[i] + k2))
